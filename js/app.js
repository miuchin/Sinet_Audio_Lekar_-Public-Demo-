...
class IosAudioSessionKeeper {
  constructor(enabled, srcUrl=null) {
  ...
  _ensureEl() {
    ...
    el.src = (this.srcUrl || SILENT_WAV_DATA_URI);
    el.loop = true;
    el.preload = "auto";
    el.playsInline = true;
  ...
  setSource(url){
    this.srcUrl = url;
    if (this.audioEl) {
      try { this.audioEl.src = url; this.audioEl.load && this.audioEl.load(); } catch(_) {}
    }
  }
...
class App {
  constructor() {
    ...
    this._iosKeeper = new IosAudioSessionKeeper(this._isIOS, 'data/silence_30s_8k.wav');
    this._iosHintShown = false;

    // iOS background policy:
    //  - SAFE (default): pause immediately when app goes to background (prevents iOS pulsing + forced stop)
    //  - EXPERIMENTAL: try to keep playing in background (may still stop on some iOS versions)
    this._iosAllowBackground = false;
    try { this._iosAllowBackground = localStorage.getItem("sinet_ios_allow_bg") === "1"; } catch(_) {}
    this._iosBgAutoPaused = false;
  ...
  async init() {
    console.log('SINET v15.4.9.2 Init');
    this.cacheUI();

    // iOS / background / lock-screen best-effort helpers

    // iOS settings UI
    this._initIosSettingsUI();
  ...
  _initIosSettingsUI() {
    try {
      const card = document.getElementById('ios-audio-card');
      if (!card) return;
      if (!this._isIOS) { card.style.display = 'none'; return; }
      const cb = document.getElementById('ios-allow-bg');
      if (cb) cb.checked = !!this._iosAllowBackground;
    } catch(_) {}
  }

  setIosAllowBackground(on) {
    this._iosAllowBackground = !!on;
    try { localStorage.setItem('sinet_ios_allow_bg', this._iosAllowBackground ? '1' : '0'); } catch(_) {}
    // Best-effort: (re)start keeper only when explicitly enabled and (optionally) playing
    try {
      if (!this._isIOS) return;
      if (!this._iosAllowBackground) {
        this._iosKeeper.stop();
        this.showToast('🍏 iOS SAFE: u pozadini se pauzira (preporučeno).', { timeoutMs: 4000 });
      } else {
        this.showToast('🍏 Eksperimentalno: pokušavamo zvuk u pozadini (iOS može ipak prekinuti).', { timeoutMs: 5000 });
        if (this.audio && this.audio.isPlaying) {
          // ensure keeper started when playback session is active
          try { this._iosKeeper.start(); } catch(_) {}
        }
      }
    } catch(_) {}
  }
  ...
  showIosAudioStatus() {
    try {
      ...
      const status = {
        version: SINET_APP_VERSION,
        isIOS: !!this._isIOS,
        allowBackground: !!this._iosAllowBackground,
        pageHidden: !!document.hidden,
        engineIsPlaying: !!this.audio?.isPlaying,
        audioContextState: ctx ? ctx.state : 'none',
        keeperStarted: !!this._iosKeeper?.started
      };
      alert(JSON.stringify(status, null, 2));
    } catch(e) {
      try { alert('iOS status error: ' + (e?.message || e)); } catch(_) {}
    }
  }
...
VAŽNO: samo JSON.`;
  ...
  _ensurePlaybackSession() {
    ...
    if (this._isIOS) {
      if (this._iosAllowBackground) this._iosKeeper.start();
  ...
  _bindBackgroundRecovery() {
    ...
    document.addEventListener("visibilitychange", () => {
      // iOS SAFE (default): pause immediately in background to avoid pulsing + forced stop.
      if (document.hidden) {
        if (this._isIOS && !this._iosAllowBackground) {
          this._iosBgAutoPaused = true;
          try { this._iosKeeper.stop(); } catch(_) {}
          try {
            if (this.audio && this.audio.isPlaying) {
              // use the same path as user pressing pause (updates UI + resume state)
              this.togglePlayPause();
            }
          } catch(_) {}
          try {
            const shown = localStorage.getItem("sinet_ios_bg_autopause_hint") === "1";
            if (!shown) {
              localStorage.setItem("sinet_ios_bg_autopause_hint", "1");
              this.showToast("🍏 iOS SAFE: zvuk je pauziran u pozadini (stabilno). Vrati se u aplikaciju i tapni ▶ da nastaviš.", { timeoutMs: 7000 });
            }
          } catch(_) {}
        }
        return;
      }
      // When visible again, try to resume context (some iOS builds suspend it).
      kick();
    });
