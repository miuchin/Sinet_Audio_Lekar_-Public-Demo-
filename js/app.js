...
import { SinetAudioEngine } from './audio/audio-engine.js?v=15.4.8.3';
import { normalizeCatalogPayload } from './catalog/stl-adapter.js';

const SINET_APP_VERSION = "15.4.8.3";
...
// Silent WAV (30s, 8kHz) — used to keep audio session alive on iOS in background.
// NOTE: Very short looping buffers can create audible "pulsing" artifacts on some iOS devices.
const SILENT_WAV_URL = "data/silence_30s_8k.wav";
...
class IosAudioSessionKeeper {
  ...
  _ensureEl() {
    ...
    el.src = `${SILENT_WAV_URL}?v=${SINET_APP_VERSION}`;
    el.loop = true;
    el.preload = "auto";
    el.playsInline = true;
...
// init duration slider
    ...
    if (freqBox) {
      const rows = freqsAll.map((f, i) => `
        <div class="freq-row">
          <div class="freq-hz">
            <label class="freq-hz-label">
              <input class="freq-toggle" type="checkbox" ${f.enabled === false ? "" : "checked"} onchange="window.app.setModalFreqEnabled(${i}, this.checked)">
              ${(f.value ?? f.hz ?? 0)} Hz
            </label>
          </div>
          <div class="freq-meta">
            <div class="freq-title"><b>#${i+1}</b> • ${this.escapeHtml(((f.svrha || f.funkcija || f.desc || f.naziv || "—").toString()))}</div>
            <div class="freq-src">Izvor: ${this.escapeHtml(this._srcToText(f.izvor) || "—")}</div>
          </div>
          <div class="freq-dur">${perMin} min</div>
        </div>
      `).join("");
    ...
    if (this._isIOS) {
      ...
      try {
        ...
        if (!dismissed && !this._iosHintShown) {
          ...
          this.showToast("⚠️ iOS može pauzirati zvuk u pozadini ili kada je ekran ugašen. Ako primetiš prekid, drži ekran uključen ili koristi 'Add to Home Screen' i pusti terapiju uz aktivnu sesiju.", {
            timeoutMs: 9000,
            actionLabel: "📘 Pomoć",
            actionNav: "help",
            action2Label: "Ne prikazuj više",
